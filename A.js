...
async function loadEnrollments() {
    ...
    try {
        ...
        enrollmentDocs.forEach(data => {
            ...
            /*
               NOTE: studentKey mismatch check removed.
               Admin approval may save studentUid as studentKey,
               while the student's local key can be username/email.
               The studentUid check above is enough to make sure
               the enrollment belongs to this student.
            */
...
window.openMockType = function(type) {
    ...
    /*
       Refresh enrollments every time the list opens,
       so a payment approved just now shows up
    ...
    */
    loadEnrollments().then(() => {
        ...
        const seriesBox =
            document.getElementById("seriesBox");

        if (
            seriesBox &&
            !seriesBox.classList.contains("hidden") &&
            !currentSeries
        ) {
            renderTypeSeries();
        }
